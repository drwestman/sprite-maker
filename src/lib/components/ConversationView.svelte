<script lang="ts">
  ...
  import type { Animation, AnimationPolishMode, Asset, AssetPack, ChatGenerationProfile, Conversation, Message, ProviderStatus, ReferenceImage, SpriteGenerationMetadata } from "$lib/types";
  ...
  let { conversation, messages, provider, availableProviders=[], imageProviders=[], customStyles=[], runningRequestId, generationStartedAt, generationRequest, activity, selectedAsset, assets, animations, packs, references, activeReferenceIds, focusedReferenceId, draftPrompt="", workspacePath, workspaceStyle, conversationStyle, animationMode, generationProfile, onSend, onCancel, onClearAsset, onEditAsset, onEditAnimation, onViewPack, onExportAsset, onExportAnimation, onConversationStyle, onAnimationMode, onGenerationProfile, onProviderSwitch, onAttachReferencePaths, onAttachReferenceFiles, onFocusReference, onRemoveReference, onDraftConsumed, onLinkError }: {
    ...
    workspaceStyle: StylePresetId; conversationStyle: ConversationStyleId; animationMode: AnimationPolishMode; generationProfile: ChatGenerationProfile; customStyles?:CustomArtStyle[];
  ...
  let providerReady = $derived(Boolean(provider && ["ready", "detected"].includes(provider.status) && (provider.id !== "ollama" || generationProfile.model.trim())));
  let projectName = $derived(projectNameFromPath(workspacePath));
  let generationActive = $derived(Boolean(sending || runningRequestId));
  let effectiveStartedAt = $derived(generationStartedAt ?? localStartedAt);
  ...
  async function chooseAnimationMode(value: AnimationPolishMode) {
    await onAnimationMode(value);
  }
  ...
  function readableContent(message: Message, hasArtifact: boolean): string {
    ...
    if (message.role !== "assistant" || !["claude", "gemini", "grok", "cursor", "antigravity"].includes(messageProviderId(message))) return value;
  ...
  function messageProviderId(message: Message): string {
    const value = message.metadata?.provider;
    return typeof value === "string" && value.trim() ? value : provider?.id ?? "agent";
  }
  function messageProviderName(message: Message): string {
    const id = messageProviderId(message);
    return availableProviders.find(item => item.id === id)?.name ?? (id === provider?.id ? provider?.name : undefined) ?? id;
  }
  function messageHandoffName(message: Message): string {
    const value = message.metadata?.handoffProvider;
    if (typeof value !== "string" || !value.trim()) return "generation provider";
    return availableProviders.find(item => item.id === value)?.name ?? value;
  }
  function providerStatusLabel(status?: string): string {
    return status === "ready" ? "Ready" : status === "detected" ? "Detected" : status === "needs_setup" ? "Needs setup — configure Ollama" : status === "offline" ? "Offline — start Ollama or update Settings" : "Unavailable";
  }
</script>

<section class="conversation-view">
  <header>
    <div class="chat-heading">{#if provider}<span class={`provider-mark ${provider.id}`}><ProviderLogo providerId={provider.id} label={provider.name} size={18}/></span>{/if}<div><h1>{conversation?.title ?? "Agent"}</h1><p>{provider?.name ?? "No provider selected"} · {providerStatusLabel(provider?.status)}</p></div></div>
    <div class="header-actions">
      {#if conversation}<details class="provider-menu" bind:this={providerDetails}><summary><span class={`provider-mark compact ${provider?.id ?? ""}`}><ProviderLogo providerId={provider?.id ?? "agent"} label={provider?.name} size={14}/></span><span>{provider?.name ?? "Choose provider"}</span><ChevronDown size={12}/></summary><div class="provider-popover"><div class="provider-popover-heading"><strong>Chat provider</strong><p>Switching keeps this chat visible and starts a fresh provider session.</p></div><div class="provider-list">{#each availableProviders.filter(item=>item.kind === "agent") as item}<button class:active={item.id===provider?.id} onclick={()=>chooseProvider(item.id)} disabled={Boolean(runningRequestId)}><span class={`provider-mark compact ${item.id}`}><ProviderLogo providerId={item.id} label={item.name} size={14}/></span><span><strong>{item.name}</strong><small>{item.status === "ready" ? "Ready to use" : item.status === "detected" ? "Detected — checked when you send" : item.detail}</small></span>{#if item.id===provider?.id}<Check size={14}/>{/if}</button>{/each}</div><div class="provider-model"><label>Model<select value={generationProfile.model} onchange={(event)=>chooseModel(event.currentTarget.value)} disabled={!provider?.modes.length || Boolean(runningRequestId)}>{#if provider?.id === "ollama"}<option value="">Choose model</option>{:else if !provider?.modes.length}<option value="">Provider default</option>{/if}{#each provider?.modes ?? [] as mode}<option value={mode.id}>{mode.label}</option>{/each}</select></label>{#if providerMode?.description}<p>{providerMode.description}</p>{/if}</div></div></details>{/if}
      ...
      {#if conversation}<AnimationPolishModeMenu value={animationMode} disabled={Boolean(runningRequestId)||!["ready","detected"].includes(provider?.status??"")} onChange={chooseAnimationMode}/>{/if}
      <div class:ready={providerReady} class="status-dot"><span></span>{provider?.status === "ready" ? "Connected" : provider?.status === "detected" ? "Detected" : providerStatusLabel(provider?.status)}</div>
    </div>
  </header>
      ...
      <div class="message-column">
        {#each messages as message}
          ...
          <article class:user={message.role === "user"} class:failed={message.status === "failed" || generationFailed}>
            <div class="avatar">{#if message.role === "user"}<span>You</span>{:else}<span class={`provider-mark compact ${messageProviderId(message)}`}><ProviderLogo providerId={messageProviderId(message)} label={messageProviderName(message)} size={14}/></span>{/if}</div>
            <div class="message-body">
              <div class="message-meta"><strong>{message.role === "user" ? "You" : message.kind === "prompt-draft" ? `Draft · ${messageProviderName(message)}` : messageProviderName(message)}</strong>{#if message.kind === "prompt-draft"}<span class="draft-handoff">→ {messageHandoffName(message)}</span>{/if}<time>{new Date(message.createdAt).toLocaleTimeString([], {hour:"2-digit",minute:"2-digit"})}</time></div>
              {#if message.content}<div class:prompt-draft={message.kind === "prompt-draft"} class="content"><MarkdownMessage content={readableContent(message,Boolean(generation||packResult))} {workspacePath} {onLinkError}/></div>{/if}
              ...
              {:else if message.kind === "prompt-draft"}<div class="message-state subtle"><Check size={11} /> Draft shown before handoff</div>
  ...
  <div class="composer-wrap">
    ...
    <div class="composer-anchor">
      ...
      {/if}
    <div class="composer" class:disabled={!providerReady}>
      ...
      <textarea bind:this={textarea} bind:value={prompt} onkeydown={keydown} onpaste={paste} rows="2" disabled={!conversation || !providerReady} placeholder={providerReady ? "Ask for a sprite, paste an image, or type / for commands…" : provider?.id === "ollama" && !generationProfile.model ? "Choose an Ollama model in Settings or the provider menu" : "Open Settings to install or sign in to this provider"}></textarea>
      <div class="composer-footer"><div class="composer-hints"><button class="attach" onclick={uploadReferences} disabled={!conversation || !providerReady || attaching} title="Attach reference images"><Paperclip size={14}/></button><span>{attaching?"Adding image…":"Paste or attach a reference"}</span></div>
        {#if runningRequestId}<button class="stop" onclick={onCancel} title="Stop request"><Square size={12} fill="currentColor" /></button>{:else}<button class="send" onclick={send} disabled={!prompt.trim() || !conversation || !providerReady || sending} title="Send message"><ArrowUp size={15} /></button>{/if}
      </div>
    </div>
    </div>
  </div>
...
<style>
  ...
  .draft-handoff{font-size:11px;color:var(--accent)}.prompt-draft{padding:12px 14px;border-left:2px solid var(--accent);border-radius:0 7px 7px 0;background:var(--accent-dim)}
</style>
