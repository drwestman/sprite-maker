<script lang="ts">
  ...
  const mfluxSelected = $derived(profile.imageProviderId === "mflux");
  ...
  function commit(value: ChatGenerationProfile) {
    const next = normalizeGenerationProfile(value, provider?.modes ?? [], provider?.id);
...
<details class="generation-menu" bind:this={menu}>
  ...
  <div class="popover">
    ...
    <div class="provider-section">
      ...
      {#if mfluxSelected}
        <div class="mflux-options">
          <div class="image-source-heading"><strong>MFLUX input</strong><span>One focused reference for image-to-image</span></div>
          <div class="segmented" aria-label="MFLUX image input mode">
            <button class:active={profile.imageInputMode === "text-to-image"} onclick={() => commit({...profile, imageInputMode: "text-to-image"})}>Text to image</button>
            <button class:active={profile.imageInputMode === "image-to-image"} onclick={() => commit({...profile, imageInputMode: "image-to-image"})}>Image to image</button>
          </div>
          {#if profile.imageInputMode === "image-to-image"}
            <label class="strength-control">Reference strength <output>{profile.imageStrength.toFixed(2)}</output><input type="range" min="0" max="1" step="0.01" value={profile.imageStrength} oninput={(event) => commit({...profile, imageStrength: Number(event.currentTarget.value)})}/></label>
            <p class="mode-description">MFLUX uses the focused reference, the only active reference, or the selected animation master.</p>
          {/if}
        </div>
      {/if}
...
<style>
  ...
  .mflux-options{margin-top:10px}.mflux-options .image-source-heading{margin-top:0}.mflux-options .segmented{display:flex;width:max-content;margin-top:9px}.mflux-options .segmented button{height:27px;min-width:108px;padding:0 9px;border:0;border-radius:4px;background:transparent;color:var(--faint);font:inherit;font-size:10px;cursor:pointer}.mflux-options .segmented button.active{background:var(--surface-hover);color:var(--text);box-shadow:0 0 0 1px var(--border-strong)}.strength-control{display:block;margin-top:11px;font-size:10px;color:var(--faint)}.strength-control output{float:right;color:var(--text);font-variant-numeric:tabular-nums}.strength-control input{height:auto;margin-top:8px;padding:0;accent-color:var(--accent)}
