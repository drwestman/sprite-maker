...
export type ProviderCapabilities = { textInput: boolean; imageInput: boolean; multipleImageInput: boolean; imageEditing: boolean; masks: boolean; transparency: boolean; structuredOutput: boolean; videoAnimation: boolean; imageToImage: boolean; maximumReferenceImages: number; toolCalling: boolean };
...
export type MfluxSettings = { repository: string; revision: string; runtimeVersion: string; runtimeReady: boolean; supportedHost: boolean; status: string; detail: string; cachePath: string };
export type MfluxSettingsInput = { repository: string; revision: string };
export type MfluxSetupEvent = { setupId: string; eventType: "started" | "progress" | "completed" | "failed" | "cancelled"; stage: string; progress: number; message: string };
export type OllamaModel = { name: string; model: string; modifiedAt?: string; size?: number; digest?: string; capabilities: string[]; family?: string; parameterSize?: string; quantizationLevel?: string; vision: boolean; structuredOutput: boolean; toolCalling: boolean; thinking: boolean };
...
export type ImageInputMode = "text-to-image" | "image-to-image";
export type ChatGenerationProfile = { profileVersion: number; quality: GenerationQuality; width: number; height: number; frames: number; fps: number; frameMode: FrameMode; minFrames: number; maxFrames: number; allowInterpolation: boolean; allowAutoAdjust: boolean; imageInputMode: ImageInputMode; imageStrength: number; model: string; reasoningEffort: string; imageProviderId: string };
export type SpriteSlashCommand = "animate" | "sprite" | "character" | "effect" | "pack" | "rig";
export type ProviderRequestOptions = {
  model?: string;
  reasoningEffort?: string;
  command?: SpriteSlashCommand;
  generation: Pick<ChatGenerationProfile, "quality" | "width" | "height" | "frames" | "fps" | "frameMode" | "minFrames" | "maxFrames" | "allowInterpolation" | "allowAutoAdjust" | "imageInputMode" | "imageStrength">;
  referenceIds?: string[];
