...
pub fn create_conversation(
    ...
    create_conversation_inner(workspace_id, worktree_id, title, provider, &state)
}

pub(crate) fn create_conversation_inner(
    workspace_id: String,
    worktree_id: Option<String>,
    title: Option<String>,
    provider: Option<String>,
...
pub(crate) fn get_message(state: &AppState, id: &str) -> CommandResult<Message> {
    let connection = state
        .db
        .lock()
        .map_err(|_| CommandError::new("database_locked", "Database lock was poisoned"))?;
    connection
        .query_row(
            "SELECT id, conversation_id, role, kind, content, status, metadata_json, created_at FROM messages WHERE id = ?1",
            [id],
            message_row,
        )
        .optional()?
...
pub fn record_chat_turn(
    ...
    add_message(&state, &conversation_id, "user", "text", &user_content, "completed")?;
    add_message(
        &state,
        &conversation_id,
        "assistant",
